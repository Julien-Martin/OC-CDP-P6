				...
				<v-dialog v-model="modal.active" persistent max-width="290">
					<v-card>
						...
						<v-card-actions>
							...
							<v-btn dark class="green" flat @click="deleteProduct">Accepter</v-btn>
						</v-card-actions>
					</v-card>
				</v-dialog>
							...
							<v-card-text>
								<v-container grid-list-md>
									<v-layout wrap>
										...
										<v-flex xs12>
											<v-text-field type="number" v-model.number="editedItem.pricettc" label="Prix TTC"></v-text-field>
										</v-flex>
										<v-flex xs12>
											<v-select v-model="editedItem.unit" :items="unit" label="Unitée" hint="Facultatif"></v-select>
										</v-flex>
									</v-layout>
								</v-container>
							</v-card-text>
