					...
					<v-stepper v-model="stepper" vertical>
							...
							<v-card v-if="stepper === 1" flat>
								<v-container grid-list-md fluid>
									<v-layout row wrap>
										...
										<v-flex xs12>
											<v-text-field type="password" v-model="signup.confirmedPassword" prepend-icon="lock"
																		v-validate="'required|confirmed:mot de passe'" data-vv-as="confirmation"
																		name="confirmation"
						...
						<v-stepper-content step="2">
							<v-card v-if="stepper === 2" flat>
								<v-container grid-list-md fluid>
									<v-layout row wrap>
										<v-flex xs12 md6>
											<v-text-field v-model="signup.name.lastname" v-validate="'required'" label="Nom" name="nom"
																		:error-messages="errors.collect('nom')"></v-text-field>
										</v-flex>
										<v-flex xs12 md6>
											<v-text-field v-model="signup.name.firstname" v-validate="'required'" label="Prénom"
																		name="prénom" :error-messages="errors.collect('prénom')"></v-text-field>
										</v-flex>
										<v-flex xs12>
											<v-text-field v-model="signup.address.street" v-validate="'required'" label="Adresse ligne 1"
										...
										<v-flex xs12>
											<v-text-field v-model="signup.address.city" v-validate="'required'" label="Ville" name="ville"
																		:error-messages="errors.collect('ville')"></v-text-field>
										</v-flex>
										<v-flex xs12>
											<v-text-field v-model="signup.address.postalcode" v-validate="'required'" label="Code postal"
																		name="code postal"
																		:error-messages="errors.collect('code postal')"></v-text-field>
										</v-flex>
										<v-flex xs12>
											<v-text-field v-model="signup.address.country" v-validate="'required'" label="Pays"
																		name="pays"
						...
						<v-stepper-content step="3">
							<v-card v-if="stepper === 3" flat>
								<v-container grid-list-xs12 fluid>
									<v-layout row wrap>
										<TextField class="xs12" v-model="signup.siret" v-validate="'required'" name="numéro de siret"
															 :error-messages="errors.collect('numéro de siret')" label="Numéro de siret"></TextField>
										<v-flex xs12>
											<v-switch class="xs12" v-model="signup.useVAT" label="Assujetti à la TVA"></v-switch>
										</v-flex>
										...
										<TextField class="xs12" v-model="signup.RM" hint="Pour les activités artisanales"
						...
						<v-stepper-content step="4">
							<v-card flat>
								...
								<p class="subheading">Vous pouvez maintenant vous connectez et utilisez l'application.</p>
							</v-card>
